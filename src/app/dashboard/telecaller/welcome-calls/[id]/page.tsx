...
export default function TelecallerWelcomeCallDetailPage() {
    ...
    return (
      <div className="p-8 min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-50">
        <div className="flex h-72 flex-col items-center justify-center gap-3">
          ...
          <p className="text-sm font-semibold text-slate-700">Loading patient details...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="p-8 min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-50">
        <div className="space-y-4 max-w-2xl">
          <Alert variant="destructive" className="border-0 bg-gradient-to-br from-rose-50 to-pink-50 shadow-lg">
            <AlertTitle className="text-rose-900 font-bold">Unable to load patient profile</AlertTitle>
            <AlertDescription className="text-rose-700">{error?.message ?? "Patient not found."}</AlertDescription>
          </Alert>
          <Button 
            variant="outline" 
            onClick={() => refetch()}
  ...
  return (
    <div className="p-8 min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-50">
      <LeadDetailsDietTheme
        lead={lead}
        backHref="/dashboard/telecaller/welcome-calls"
        backLabel="Back to Welcome Call Management"
      />
    </div>
  )
}
