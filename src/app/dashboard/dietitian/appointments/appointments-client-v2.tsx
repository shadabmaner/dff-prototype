...
export default function AppointmentsClient() {
  ...
  const [pendingActionAppointment, setPendingActionAppointment] = useState<DietitianAppointment | null | any>(null)
  ...
  console.log(rescheduleForm?.appointmentDate, ">>>>>>>>>")
  ...
  const confirmStartSession = async () => {
    ...
    try {
      ...
      setShowStartSessionDialog(false)
      router.push(`/dashboard/dietitian/consultation/${pendingActionAppointment.patient_id}?name=${encodeURIComponent(patientName)}`)
  ...
  const handleViewProfile = (appointment: DietitianAppointment) => {
    ...
    router.push(`/dashboard/dietitian/patients/${appointment.patient_id}?ref=appointments&tab=${selectedTab}&name=${encodeURIComponent(name)}`)
  }
  ...
  const isAppointmentTimeValid = (appointment: any) => {
    const timeValue = getAppointmentTimeValue(appointment)
    if (!timeValue || !appointment.appointment_date) return false
    ...
    try {
      ...
      // Enable button if within 2 minutes before start time or after start time
          ...
          <div className="flex gap-1 -mb-px">
            <button
              onClick={() => {
                ...
                setPage(1)
                router.replace(`/dashboard/dietitian/appointments?tab=confirmed`, { scroll: false })
              }}
            ...
            <button
              onClick={() => {
                ...
                setPage(1)
                router.replace(`/dashboard/dietitian/appointments?tab=upcoming`, { scroll: false })
              }}
            ...
            <button
              onClick={() => {
                ...
                setPage(1)
                router.replace(`/dashboard/dietitian/appointments?tab=completed`, { scroll: false })
              }}
            ...
            <button
              onClick={() => {
                ...
                setPage(1)
                router.replace(`/dashboard/dietitian/appointments?tab=missing`, { scroll: false })
              }}
                        ...
                        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {appointment.call_status === "started" ? (
                              <Button
                                size="sm"
                                onClick={() => router.push(`/dashboard/dietitian/consultation/${appointment.patient_id}?name=${encodeURIComponent(getAppointmentPatientName(appointment))}`)}
                              ...
                              // disabled={hasActiveCall || !isAppointmentTimeValid(appointment)}
                              // className={`font-semibold shadow-lg shadow-slate-900/20 h-10 px-4 ${hasActiveCall || !isAppointmentTimeValid(appointment)
                            ...
                            {appointment?.call_type !== "history_call" && appointment.call_status !== "started" && <Button
                              size="sm"
                              variant="outline"
                              onClick={() => handleReschedule(appointment)}
                              className="border-blue-300 text-blue-600 hover:bg-blue-50 hover:border-blue-400 font-semibold shadow-sm h-10 px-4"
