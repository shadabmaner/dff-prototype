...
export default function StaffDetailPage() {
  ...
  const assignedPatientCount = stats?.total_assigned_patients ?? staff.current_patient_count ?? assignedPatients?.meta?.total ?? 0;

  const capacityPercent = staff.max_patients && staff.max_patients > 0
  ...
  const professionalItems = [
    ...
    staff.consultation_fee != null && { label: "Consultation Fee", value: `₹${staff.consultation_fee}` },
    staff.bio && { label: "Bio", value: staff.bio },
  ].filter(Boolean) as { label: string; value: string }[];

  const statCards = [
    { label: "Assigned Patients", value: assignedPatientCount },
  ...
  const detailedStats = [
    { label: "Total Assigned Patients", value: assignedPatientCount },
    { label: "Total Appointments", value: stats?.total_appointments ?? 0 },
    { label: "Upcoming", value: stats?.upcoming_appointments_count ?? 0 },
    { label: "Completed", value: stats?.completed_appointments_count ?? 0 },
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-7xl space-y-8 p-6 lg:p-8">
        ...
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-2 text-sm text-slate-500">
              ...
              <span>/</span>
              <span className="text-slate-900 font-medium">Staff Details</span>
            </div>
          </div>
        </div>
        ...
        {/* ── Patients & Appointments (Full Width Tabs) ── */}
        <Card className="border-slate-200 shadow-sm">
