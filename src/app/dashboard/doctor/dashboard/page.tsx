...
export default function DoctorDashboard() {
  const { data, isLoading, isFetching, error, refetch } = useQuery<DoctorDashboardResponse, Error>({
    queryKey: ["doctor-dashboard"],
    queryFn: async () => {
      try {
        const response = await apiClient.get<DoctorDashboardEnvelope | DoctorDashboardResponse>("/dashboards/doctor")
        ...
        if (payload && typeof payload === "object" && "totalAssignedPatients" in payload) {
          return payload as DoctorDashboardResponse
        }

        throw new Error("Unable to load doctor dashboard")
      } catch (err: any) {
    ...
    <div className="space-y-6 p-8 min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-50 rounded-[50px]">
      <div className="space-y-4">
        <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold text-slate-900 tracking-tight">Doctor Dashboard</h1>
