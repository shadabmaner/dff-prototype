...
import { useDoctorAppointments, useDoctorUpcomingAppointments, useStaffSlots } from "@/hooks/use-doctor"
import { useQueryClient } from "@tanstack/react-query"
import type { DoctorAppointment, RescheduleAppointmentRequest } from "@/types/doctor-clinical"
...
export default function AppointmentsClient() {
  ...
  const [pendingActionAppointment, setPendingActionAppointment] = useState<DoctorAppointment | null | any>(null)
  ...
  // Update appointments when response data changes (using actual response dependencies, not computed value)
  useEffect(() => {
    setAppointments(appointmentsData)
  }, [selectedTab, completedResponse, upcomingResponse, missingResponse, appointmentsResponse])

  // Force fresh data when tab changes to prevent showing old cached data
  useEffect(() => {
    if (selectedTab !== currentTab) {
      // Force refetch to get fresh data, not cached data
      queryClient.refetchQueries({ queryKey: ["doctor-appointments"] })
      queryClient.refetchQueries({ queryKey: ["doctor-upcoming-appointments"] })
      queryClient.refetchQueries({ queryKey: ["doctor-missing-appointments"] })
    }
  ...
  const confirmStartSession = async () => {
    ...
    try {
      ...
      setShowStartSessionDialog(false)
      router.push(`/dashboard/doctor/consultation/${pendingActionAppointment.patient_id}?name=${encodeURIComponent(patientName)}`)
  ...
  const handleAddConsultationLink = (appointment: DoctorAppointment) => {
  ...
  const getAppointmentPatientName = (appointment?: DoctorAppointment | null) => {
  ...
  const handleViewProfile = (appointment: DoctorAppointment) => {
    const name = getAppointmentPatientName(appointment)
    router.push(`/dashboard/doctor/patients/${appointment.patient_id}?ref=appointments&tab=${selectedTab}&name=${encodeURIComponent(name)}`)
  }

  const getAppointmentTimeValue = (appointment: DoctorAppointment) => {
    return (
    ...
    <div className="space-y-6 p-8 min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-50 rounded-[50px]">
      ...
      <Card className="border border-slate-200/80 bg-white/90 backdrop-blur-sm shadow-lg">
          ...
          <div className="flex gap-1 -mb-px">
            <button
              onClick={() => {
                ...
                setPage(1)
                router.replace(`/dashboard/doctor/appointments?tab=confirmed`, { scroll: false })
              }}
            ...
            <button
              onClick={() => {
                ...
                setPage(1)
                router.replace(`/dashboard/doctor/appointments?tab=upcoming`, { scroll: false })
              }}
            ...
            <button
              onClick={() => {
                ...
                setPage(1)
                router.replace(`/dashboard/doctor/appointments?tab=completed`, { scroll: false })
              }}
            ...
            <button
              onClick={() => {
                ...
                setPage(1)
                router.replace(`/dashboard/doctor/appointments?tab=missing`, { scroll: false })
              }}
            ...
            <button
              onClick={() => {
                setSelectedTab("history_calls")
                setPage(1)
                router.replace(`/dashboard/doctor/appointments?tab=history_calls`, { scroll: false })
              }}
              className={`px-4 py-3 text-sm font-medium transition-colors border-b-2 ${selectedTab === "history_calls"
        ...
        <CardContent className="p-0">
                        ...
                        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {appointment.call_status === "started" ? (
                              <Button
                                size="sm"
                                onClick={() => router.push(`/dashboard/doctor/consultation/${appointment.patient_id}?name=${encodeURIComponent(getAppointmentPatientName(appointment))}`)}
          ...
          {/* History Calls Tab Content */}
          {selectedTab === "history_calls" && (
            <div className="space-y-4">
              {isLoading ? (
                <Card className="border border-slate-200/80 bg-white/80 backdrop-blur-sm shadow-lg rounded-xl">
                  <CardContent className="p-10 space-y-4">
                    {Array.from({ length: 3 }).map((_, idx) => (
                      <div key={`history-skeleton-${idx}`} className="h-16 w-full animate-pulse rounded-lg bg-slate-100" />
                    ))}
                  </CardContent>
                </Card>
              ) : (
                <div className="space-y-3">
                  {/* Static history call data */}
                  {[
                    { id: 1, patientName: "Rahul Sharma", date: "2024-06-15", time: "10:00 AM", status: "confirmed" },
                    { id: 2, patientName: "Priya Patel", date: "2024-06-16", time: "11:30 AM", status: "pending" },
                    { id: 3, patientName: "Amit Kumar", date: "2024-06-17", time: "2:00 PM", status: "confirmed" },
                  ].map((historyCall) => (
                    <Card key={historyCall.id} className="border border-slate-200/80 bg-white/80 backdrop-blur-sm shadow-lg rounded-xl hover:shadow-md transition-shadow cursor-pointer"
                      onClick={() => router.push(`/dashboard/doctor/patients/${historyCall.id}?first_name=${encodeURIComponent(historyCall.patientName.split(' ')[0])}&last_name=${encodeURIComponent(historyCall.patientName.split(' ')[1])}`)}
                    >
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
                            <div className="h-10 w-10 rounded-full bg-gradient-to-br from-purple-500 to-indigo-500 flex items-center justify-center text-white text-sm font-bold">
                              {historyCall.patientName.split(' ').map(n => n[0]).join('')}
                            </div>
                            <div>
                              <h4 className="font-semibold text-slate-900">{historyCall.patientName}</h4>
                              <p className="text-sm text-slate-600">{historyCall.date} at {historyCall.time}</p>
                            </div>
                          </div>
                          <Badge className={
                            historyCall.status === "confirmed" 
