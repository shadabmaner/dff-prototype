...
export function PatientRequestList({ appointments, isLoading, onConfirm, onReschedule, onCancel, hideActions }: PatientRequestListProps) {
  ...
  const getReason = (request: DietitianAppointment) => request.reason || "Consultation request"
  ...
  const handleViewProfile = (request: DietitianAppointment) => {
    ...
    router.push(`/dashboard/fitness-coach/patients/${request.patient_id}?ref=patient-requests&name=${encodeURIComponent(name)}`)
  }
