...
import { Dumbbell, CheckCircle2, Calendar, Clock, Activity } from "lucide-react";
...
export function ExerciseLogsCard({ exerciseLogs }: ExerciseLogsCardProps) {
  return (
        ...
        {exerciseLogs.length > 0 ? (
          <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
            {exerciseLogs.map((log: any, index: number) => (
              <div
                key={log.id || index}
                className="p-4 rounded-xl border border-slate-200 bg-slate-50 relative overflow-hidden"
              >
                {log.is_completed && (
                ...
                <div className="flex justify-between items-center mb-2">
                  <div className="flex-1">
                    <p className="text-[10px] text-slate-500 font-medium uppercase tracking-wider mb-0.5">
                      {log.collection_name}{" "}
                      {log.day_number ? `• Day ${log.day_number}` : ""}
                    </p>
                    <p className="font-bold text-slate-800 leading-tight">
                      {log.title || "Exercise Session"}
                    </p>
                  </div>
                  <Badge
                    variant="outline"
          ...
          <div className="text-center py-8 bg-slate-50 rounded-xl border border-dashed border-slate-200">
            ...
            <p className="text-slate-500 text-sm">
              No exercise logs found for this patient.
            </p>
          </div>
        )}
      </CardContent>
